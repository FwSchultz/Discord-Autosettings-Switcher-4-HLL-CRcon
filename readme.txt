...
Erforderliche DC-Bot-Rechte:
Grundlegende Rechte:

Nachrichten senden (Send Messages): Damit der Bot Nachrichten in den Kanälen posten kann.
Nachrichten lesen (Read Messages): Damit der Bot auf Nachrichten und Befehle reagieren kann.
Links einbetten (Embed Links): Falls der Bot eingebettete Nachrichten (Embeds) nutzt, z. B. für Statusinformationen.
Dateien anhängen (Attach Files): Für den Fall, dass JSON-Daten oder Logs als Datei gesendet werden.
Erweiterte Rechte:

Nachrichten verwalten (Manage Messages): Falls der Bot alte Nachrichten löschen oder aktualisieren muss.
Reaktionen hinzufügen (Add Reactions): Falls der Bot Reaktionen auf Nachrichten verwendet.
Admin-/Slash-Befehl-spezifische Rechte:

Befehle verwenden (Use Application Commands): Damit Benutzer Slash-Befehle wie /apply_settings oder /status ausführen können.
Optionale Rechte (abhängig vom Anwendungsfall):

Erwähnungen verwenden (Mention @everyone, @here, and All Roles): Falls der Bot Gruppen oder alle Benutzer in einem Kanal erwähnen soll.